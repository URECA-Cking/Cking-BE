...
| 57 | 내부 | Drawing | INTERNAL | CALL | `DrawingPublicationService.publish(drawingId, adminId)` | INITIAL Drawing 공개와 Event `DRAW_COMPLETED→PUBLISHED` 전이 | 상태 기반 |
...
No. 9, No. 50~60은 외부 API가 아니라 내부 Service Method이므로 외부 API 수에 포함하지 않는다.
