...
| 57 | 내부 | 공개 | INTERNAL | CALL | `PublicationService.publish(drawingId, adminId)` | INITIAL Drawing 공개와 Winner Notification 생성을 하나의 Transaction으로 처리 | 상태 기반 |
| 58 | 내부 | Drawing | INTERNAL | CALL | `DrawingPublicationService.publish(drawingId, adminId)` | INITIAL Drawing 공개와 Event `DRAW_COMPLETED→PUBLISHED` 전이 | 상태 기반 |
