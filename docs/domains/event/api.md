...
시스템4의 INITIAL 결과 공개 Transaction은 `EventCommandService.publish(eventId)`를 호출한다. 이 메서드는 Event 행의
...
Bearer Access JWT가 필수이며 호출자는 `@CurrentMemberId`로 식별한다. 공개 Event 정보와 해당 Event의 Creator 기준 요청 사용자의 보유 응모권 수를
