...
- **알려진 제약(미해결)**: 공용 EARN의 Dead Stream 이관·replay(위 PEL 회수의 영구 실패 메시지 정리, [이슈 #244](https://github.com/URECA-Cking/Cking-BE/issues/244))와 잔액 수동 보정(`TicketCompensationService`/`ticket:maint:` maintenance lock 대응)이 아직 없다. `BALANCE_MAINTENANCE`도 아직 반환되지 않는다 — 후속 이슈로 남긴다.
