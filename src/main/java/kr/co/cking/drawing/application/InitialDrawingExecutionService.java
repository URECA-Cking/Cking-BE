...
public class InitialDrawingExecutionService {
    ...
    private void saveWinners(Drawing drawing, DrawOutput output) {
        List<Winner> winners = output.winners().stream()
                .sorted(Comparator.comparingInt(DrawWinner::rank))
                .map(winner -> toWinner(drawing, winner))
                .toList();
    ...
    private Winner toWinner(Drawing drawing, DrawWinner winner) {
