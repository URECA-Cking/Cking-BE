 ...
 * (요구사항 v4 FR-14a). 보존해 둔 원본 payload를 그대로 재적용하고, 성공하면
 * RESOLVED로 표시한다 — {@link TicketEarnLedgerService#apply}가 requestId 기준으로
 * 멱등하므로 중복 replay를 눌러도 안전하다.
 */
@Service
...
public class DeadStreamReplayService {
    ...
    public void replay(Long deadStreamMessageId, Long resolvedBy) {
        ...
        if (message.getStreamType() != DeadStreamType.EARN) {
            throw new UnsupportedOperationException("EARN 외 Stream 타입의 replay는 아직 지원하지 않습니다. type=" + message.getStreamType());
        }

        ticketEarnLedgerService.apply(EarnCommand.fromStreamFields(readPayload(message.getPayload())));
