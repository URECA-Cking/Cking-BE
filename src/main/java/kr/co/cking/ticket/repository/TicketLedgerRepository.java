...
public interface TicketLedgerRepository extends JpaRepository<TicketLedger, Long> {
    ...
    @Query(value = """
            ...
            left join mission_completion mc on mc.completion_id = l.mission_completion_id
            left join event_entry ee on ee.entry_id = l.event_entry_id
            where l.member_id = :memberId and l.creator_id = :creatorId
            order by l.created_at desc, l.ledger_id desc
            """, nativeQuery = true)
    List<TicketLedgerView> findFirstPageView(@Param("memberId") Long memberId,
    ...
    @Query(value = """
            ...
            order by l.created_at desc, l.ledger_id desc
            """, nativeQuery = true)
    List<TicketLedgerView> findAfterCursorView(@Param("memberId") Long memberId,
                                               @Param("creatorId") Long creatorId,
