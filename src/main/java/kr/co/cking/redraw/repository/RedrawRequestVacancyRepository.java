...
public interface RedrawRequestVacancyRepository extends JpaRepository<RedrawRequestVacancy, Long> {
    ...
    /** 진행 중·Retry 대기·실행 완료 요청이 점유한 Winner ID를 조회한다. */
    @Query("""
            ...
            where vacancy.winnerId in :winnerIds
              and (
                  ...
                  or request.executionStatus in (
                      kr.co.cking.redraw.domain.RedrawExecutionStatus.FAILED,
                      kr.co.cking.redraw.domain.RedrawExecutionStatus.EXECUTED
                  )
              )
            """)
