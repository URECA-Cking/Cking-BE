...
class DeadStreamAdminControllerTest {
    ...
    void status_page_size를_전달한다() throws Exception {
        ...
        mockMvc.perform(get("/api/admin/dead-streams")
                        .param("status", "RESOLVED").param("page", "2").param("size", "50"))
                .andExpect(status().isOk());
    ...
    void 잘못된_입력은_서비스_호출_없이_400이다() throws Exception {
        mockMvc.perform(get("/api/admin/dead-streams").param("size", "101"))
                .andExpect(status().isBadRequest());
        mockMvc.perform(get("/api/admin/dead-streams").param("status", "UNKNOWN"))
                .andExpect(status().isBadRequest());
        mockMvc.perform(post("/api/admin/dead-streams/0/replay"))
                .andExpect(status().isBadRequest());
    ...
    @Test
    void 없는_메시지는_404다() throws Exception {
